...
export SSI_NAME="ROR-Firewall-SSI"
...
export SPLUNK_SOURCE="ror-firewall-ssi"
export SPLUNK_SOURCE_TYPE="ror-firewall-ssi:high"
```
...
│   └── argo-ror-firewall-ssi.yaml.example # Argo CD Application example
├── logs/                     # Log files directory (auto-created)
├── charts/                   # Helm charts
│   └── dcn-ror-firewall-ssi/ # Production Helm chart
...
│   └── ror-firewall-ssi.yaml # Pod deployment
└── ssi/                      # Source code
...
- See `charts/ror-firewall-ssi/README.md` for complete documentation

**Argo CD Deployment:**

```bash
# Apply Argo CD Application manifest
kubectl apply -f examples/argo-ror-firewall-ssi.yaml.example
```
...
# Build the image
docker build -t ror-firewall-ssi:latest .
...
kubectl apply -f kubernetes/ror-firewall-ssi.yaml

# Check status
kubectl get pods -n ssi
kubectl logs -n ssi ror-firewall-ssi

# Update configuration
kubectl edit configmap ror-firewall-ssi-config -n ssi
...
docker exec ror-firewall-ssi id
# Should output: uid=1993(deno) gid=1993(deno)

# View logs (logs are stored inside container)
docker logs ror-firewall-ssi
docker logs -f ror-firewall-ssi  # Follow logs in real-time
```
...
docker inspect ror-firewall-ssi | grep -A 10 Mounts
...
# - Verify securityContext in ror-firewall-ssi.yaml
# - Ensure ConfigMap and Secret are properly mounted
kubectl describe pod ror-firewall-ssi -n ssi
kubectl logs ror-firewall-ssi -n ssi
```
...
4. **Run with Docker:**
   ...
   docker-compose up -d
   # View logs
   docker logs -f ror-firewall-ssi
   ```
