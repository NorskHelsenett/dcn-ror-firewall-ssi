/**
 * Logger Configuration - Winston-based logging system for ROR-Firewall-SSI
...
logger.on("error", (error: Error) => {
  logger.warning(`ror-firewall-ssi: Error in logger ${error.message}`, {
    component: "logger",
...
const addHecLogger = () => {
  try {
    ...
    logger.error(
      `ror-firewall-ssi: Error on WinstonHecLogger,  ${
...
export const removeHecLogger = () => {
  try {
    ...
    logger.error(
      `ror-firewall-ssi: Error on removeHecLogger,  ${
        (error as Error).message
      }`,
      {
        component: "logger",
...
export const addFileLoggers = () => {
  try {
    ...
    logger.error(
      `ror-firewall-ssi: Error on FileLogger,  ${(error as Error).message}`,
      {
        component: "logger",
...
export const removeFileLoggers = () => {
  try {
    ...
    logger.error(
      `ror-firewall-ssi: Error on removeFileLogger,  ${
        (error as Error).message
      }`,
      {
        component: "logger",
...
export const addSplunkFileLogger = () => {
  try {
    ...
    logger.error(
      `ror-firewall-ssi: Error on SplunkFileLogger,  ${
        (error as Error).message
      }`,
      {
        component: "logger",
...
export const removeSplunkFileLogger = () => {
  try {
    ...
    logger.error(
      `ror-firewall-ssi: Error on removeSplunkFileLogger,  ${
        (error as Error).message
      }`,
      {
        component: "logger",
...
logger.debug(`ror-firewall-ssi: Logger initialized at ${logLevel()} level`);
